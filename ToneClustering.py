...
class WordClustering:
    ...
    def node_weights(self, cluster_distance, nodes):
        node_list = sorted([node for node in nodes], key=lambda x: bin(x).count("1"), reverse=True)
        node_weight = [bin(node).count("1") for node in node_list]
        for nw in node_weight:
            if nw > 1:
                print(nw)
